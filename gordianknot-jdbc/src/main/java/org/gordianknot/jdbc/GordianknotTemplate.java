...
import org.gordianknot.jdbc.keygen.DefaultKeyGenerator;
import org.gordianknot.jdbc.keygen.KeyGenerator;
import org.gordianknot.jdbc.keygen.KeyGeneratorFactory;
import org.gordianknot.jdbc.util.ArrayUtils;
import org.gordianknot.jdbc.util.BeanUtils;
import org.gordianknot.jdbc.util.ClassReadUtils;
import org.gordianknot.jdbc.util.ClassUtils;
import org.gordianknot.jdbc.util.ReflectUtils;
 ...
 */
public class GordianknotTemplate extends JdbcTemplate {
 	private IDBHelper dbHelper = new MySQLHelper();
 	private static final Logger logger = LoggerFactory.getLogger(GordianknotTemplate.class);
 	private KeyGenerator keyGenerator = KeyGeneratorFactory.createKeyGenerator(DefaultKeyGenerator.class);
 	
 	public GordianknotTemplate() {
 		
 	}
 	
 	public GordianknotTemplate(String...poPackages) {
 		for (String pck : poPackages) {
 			try {
 				Set<Class<?>> classList = ClassReadUtils.getClassFromPackagePath(pck);
 				for (Class<?> clazz : classList) {
 					String className = clazz.getName();
 					// 存儲類信息，用於後面RowMapperFactory中判斷是否已經存儲過映射信息
 					...
 					for (Field field : fields) {
 						...
 						if (field.isAnnotationPresent(org.gordianknot.jdbc.annotation.Field.class)) {
 							org.gordianknot.jdbc.annotation.Field cf = field.getAnnotation(org.gordianknot.jdbc.annotation.Field.class);
 	...
 	/**
 	* <h3>保存一個實體對象</h3>
 	* <p>此方法用反射機制動態生成insert語句，類名即是表名</p>
 	* <p>通常一個自增主鍵的表，插入記錄的時候，需要指定排除主鍵</p>
 	* @param entityClass
 	* @param entity
 	* @param excludeFields 要排除的字段
 	* @return 主鍵，如果數據庫不支持java.sql.PreparedStatement.RETURN_GENERATED_KEYS，則返回null
 	*/
 	...
 	private int[] doBatchUpdate(Class<?> entityClass, List<?> entitys, String pkField, List<Object[]> allValues,
 		...
 		if(fieldNames.length == 0){
 			throw new IllegalArgumentException("請設置要修改的字段，不能全部排除掉");
 		}
 		String tableName = getTableName(entityClass);
 	...
 	/**
 	* <h3>更新一個實體對象</h3>
 	* <p>此方法用反射機制動態生成update語句，類名即是表名，只用於根據主鍵定位記錄的更新</p>
 	* <p>主鍵會自動排除，不用再指定排除主鍵，當你這麼做時，也不會有錯誤</p>
 	* @param entityClass
 	* @param entity
 	* @param pkField 主鍵
 	* @param excludeFields 要排除的字段
 	*/
 	...
 	/**
 	* <h3>根據主鍵刪除一個實體對象</h3>
 	* <p>此方法用反射機制動態生成delete語句，類名即是表名</p>
 	* @param entityClass
 	* @param pkField 主鍵名
 	* @param pk 主鍵值
 	*/
 	...
 	public <T> void deleteByParams(Class<T> entityClass, String[] params, Object[] values) {
 		if (params == null || params.length == 0) throw new RuntimeException("params必須有值");
 		String tableName = getTableName(entityClass);
 		StringBuilder sql = new StringBuilder(Constants.DELETE_SQL).append(tableName)
 				.append(Constants.ONE_EQ_ONE_SQL);
 		for (int i = 0; i < params.length; i++) {
 	...
 	*/
 	...
 	/**
 	* 根據值的定義類型調用PreparedStatement相應的set方法，將值加入到SQL參數中
 	* @param ps PreparedStatement實例
 	* @param value 準備加入到SQL參數中的值
 	* @param index 當前參數索引
 	* @throws SQLException
 	*/
 	...
 	private void printSqlLog(String sql, Object... values) {
 		if (System.getProperty("gordianknot.show.sql") != null) {
