...
public class RowMapperFactory {
 	...
 	private static class RowMapperImpl<T> implements RowMapper<T> {
 		...
 		public RowMapperImpl(Class<T> entityClass, String[] excludeFields){
 			...
 			// 第一次使用初始換映射關係，或者配置GordianknotJdbcTemplate時指定po對應的包進行映射
 		...
 		* 得到正確的值
 		* @param rs
 		* @param name 數據庫返回結果集的列名，與實體字段名對應
 		* @param type 定義的數據類型
 		* @return
 		* @throws SQLException
 		*/
