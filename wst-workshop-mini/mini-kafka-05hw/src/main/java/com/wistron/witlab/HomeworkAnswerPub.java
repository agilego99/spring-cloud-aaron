...
public class HomeworkAnswerPub {
    ...
    public static void main(String[] args) {
        ...
        String topicName = "ak05.ws" + WORKSHOP_ID + ".homework"; // 個人作業繳交的Topic
        ...
        try {
            ...
            producer.send(new ProducerRecord<>(topicName, STUDENT_ID+"|1",  "3"));  // 第1題
            producer.send(new ProducerRecord<>(topicName, STUDENT_ID+"|2",  "3"));  // 第2題
            producer.send(new ProducerRecord<>(topicName, STUDENT_ID+"|3",  "2"));  // 第3題
            producer.send(new ProducerRecord<>(topicName, STUDENT_ID+"|4",  "1"));  // 第4題
            producer.send(new ProducerRecord<>(topicName, STUDENT_ID+"|5",  "3"));  // 第5題
            producer.send(new ProducerRecord<>(topicName, STUDENT_ID+"|6",  "2"));  // 第6題
            producer.send(new ProducerRecord<>(topicName, STUDENT_ID+"|7",  "3"));  // 第7題
            producer.send(new ProducerRecord<>(topicName, STUDENT_ID+"|8",  "1"));  // 第8題
            producer.send(new ProducerRecord<>(topicName, STUDENT_ID+"|9",  "4"));  // 第9題
            producer.send(new ProducerRecord<>(topicName, STUDENT_ID+"|10", "4")); // 第10題
